...
# Carregar as variáveis de ambiente do arquivo .env, a API do Groq
load_dotenv()
...
class GroqClient:
    ...
    def generate_score(self, cv, job, max_attempts=10):
        ...
        prompt = f'''
            ...
            **Instruções IMPORTANTES:**
            ...
            - A nota FINAL deve ser obrigatoriamente um número entre 0.0 e 10.0 (inclusive).  
            - NÃO coloque números acima de 10.0 ou abaixo de 0.0.
            - Formato de saída: apenas "Pontuação Final: x.x", sem textos extras.
            ...
            **Atenção:** Seja rigoroso ao atribuir as notas. A nota máxima é 10.0, e o output deve conter apenas "Pontuação Final: x.x". O valor nunca deve ser maior que 10.0 e nem menor que 0.0.
    ...
    def extract_score_from_result(self, result_raw):
        ...
        # Regex ajustado para capturar somente números válidos
        pattern = r"(?i)Pontuação Final[:\s]*([0-9]+(?:[.,][0-9]+)?)"
        ...
        if match:
            ...
            try:
                ...
                if score > 10.0:
                    print(f"[DEBUG] Score maior que 10 recebido ({score}), truncando para 10.0.")
                    score = 10.0
                elif score < 0.0:
                    print(f"[DEBUG] Score menor que 0 recebido ({score}), truncando para 0.0.")
                    score = 0.0
                return score
            except ValueError:
                print(f"[DEBUG] Valor capturado para score foi inválido: '{score_str}'. Resposta bruta: {result_raw}")
        else:
            print(f"[DEBUG] Nenhuma pontuação encontrada. Resposta bruta: {result_raw}")
        return None
